...
\begin{table}
\caption{Standard deviations (first row) and p-values (second row, $\chi^2(2)$) for random effects in Exp.~1a model.}
...
\begin{table}
\caption{Standard deviations (first row) and p-values (second row, $\chi^2(2)$) for random effects in Exp.~1b model.}
\begin{tabular}{c c c c c c }
\toprule
...
Mean projectivity ratings obtained in Exp.~1b are shown as a function of their mean not-at-issueness ratings obtained in Exp.~2b in \figref{fig:f-proj-ai-2b}. While there appears to be an overall increase in projectivity with decreasing at-issueness, the relationship is clearly not linear as in the previous experiments. Indeed, a mixed effects linear regression predicting mean projectivity from mean at-issueness and random by-target-expression intercepts (the only random effect term justified in likelihood ratio tests) yielded no significant effect of at-issueness ($\beta$ = 0.03, $SE$ = 0.04, $t$ = 0.83, $\chi^2(1)$ = 0.68, $p >$ .4). 
...
\jd{not sure how to discuss this result. though at-issueness is signiifcant without random effects, the fact that random by-trigger intercepts important means we can't just throw these out. also, whatever the effect is, it is clearly NOT linear. but since we don't have any theoretical reason for exploring any non-linearities, do we just leave it at that, and point out that at least qualitatively, the most at-issue was also the least projecty, and the least at-issue was also at projectivity ceiling?}
