...
ggplot(d, aes(x=workerid,y=Answer.time_in_minutes)) +
  geom_point() + 
  geom_text(aes(label=workerid), vjust = 1)+
  ylim(0, 5)
...
# Bonferroni-corrected pairwise comparisons suggests no difference in at-issueness between NRRC, annoyed, NomApp, possNP, and know. But the others are all different from each other (except that know is not different from discover and stupid, stop isn't different from only, and discover isn't different from stupid)
tmp_ai$trigger = factor(x=as.character(tmp_ai$short_trigger),levels=c("NRRC","annoyed","NomApp","possNP","know","stop","discover","stupid","only"))
pairwise.t.test(tmp_ai$response, tmp_ai$trigger, p.adj = "bonf")
...
ggplot(tmp_ai, aes(x=response,fill=trigger)) +
  ...
  # geom_histogram(alpha=.5,position="identity",binwidth=.01)

ggplot(tmp_proj, aes(x=response,fill=trigger)) +
  geom_density(alpha=.5)
# geom_histogram(alpha=.5,position="identity",binwidth=.01)
...
ggplot(variances, aes(x=ProjMean,y=AIMean)) +
  geom_point() +
  # geom_errorbar(aes(ymin=AIMean-AI.ci.low,ymax=AIMean+AI.ci.high)) +
  # geom_errorbarh(aes(xmin=ProjMean-Proj.ci.low,xmax=ProjMean+Proj.ci.high)) +
  geom_smooth(method="lm") +
  geom_abline(intercept=0,slope=1, color="gray") +
  xlab("Projective mean") +
  ylab("At-issue mean")
