...
discover <- droplevels(subset(t_nomc,t_nomc$Trigger == "discover"))
discover
nrow(discover)
cor(discover$projective,discover$ai) #.43

# SALT 2020 talk
ggplot(discover, aes(x=ai,y=projective)) +
  # geom_abline(intercept=0,slope=1,linetype="dashed",color="gray50") +
  ...
  scale_y_continuous(breaks=c(0,.5,1),labels=c(0,.5,1)) +
  coord_fixed(ratio = 1)
ggsave("../graphs/subj-proj-ai-discover.pdf",width=4,height=4)
